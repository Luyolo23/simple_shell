 ...
 *
 * @data: data for directory
 * @msg: message to print
 * @error: output message
 * @ver_str: counter lines
 * Return: error message
 */
char *strcat_cd(data_shell *data, char *msg, char *error, char *ver_str)
{
char *illegal_flag;
...
{
_strcat(error, data->arguments[1]);
}
...
{
msg = ": can't cd to ";
len_id = _strlen(data->arguments[1]);
}

length = _strlen(data->av[0]) + _strlen(data->arguments[0]);
...
/**
 * error_not_found - generic error for command not found
 * @data: data relevant
 * Return: Error message
 */
...
ver_str = custom_itoa(data->custom_counter);
length = _strlen(data->av[0]) + _strlen(ver_str);
length += _strlen(data->arguments[0]) + _strlen(data->arguments[1]) + 23;
error = malloc(sizeof(char) * (length + 1));
if (error == 0)
{
free(ver_str);
